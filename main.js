...
export default async function run(client, context) {
	...
	for (const repo of repos) {
		...
		const transform = (content) => {
			const renderedByName = Object.fromEntries(
				templates.map((t) => [
					t.name,
					render(t.source, context_, { maxLength: config.maxValueLength }),
				]),
			);
	...
	await report(results, config.dryRun);
}
 ...
 * @returns {Promise<void>}
 */
async function report(results, dryRun) {
	const pullRequests = results
		.filter((r) => r.url)
		.map((r) => ({ repo: r.repo, url: r.url, number: r.number }));
	...
	core.summary
		.addHeading("Weaver sync", 2)
		.addRaw(
			`Opened: ${summary.opened} · Updated: ${summary.updated} · Skipped: ${summary.skipped}` +
				`${dryRun ? ` · Dry-run: ${summary.dryRun}` : ""} · Failed: ${summary.failed}`,
		);
	...
	if (summary.failed > 0) {
		core.setFailed(`Weaver failed for ${summary.failed} repo(s).`);
	}
}
